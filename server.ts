...
interface AppState {
  ...
  leadCaptures: LeadCaptureEntry[];
}

interface LeadCaptureEntry {
  id: string;
  email: string;
  source: string;
  createdAt: string;
...
let leadCaptures: LeadCaptureEntry[] = [];
...
const PUBLIC_DEMO_ACTION_LIMIT = Number(process.env.UACP_PUBLIC_DEMO_ACTION_LIMIT || "2");
const LEAD_CAPTURE_MAX = Number(process.env.LEAD_CAPTURE_MAX || "5000");
const LEAD_CAPTURE_MAX_SAFE = Number.isFinite(LEAD_CAPTURE_MAX) && LEAD_CAPTURE_MAX > 0
  ? Math.floor(LEAD_CAPTURE_MAX)
  : 5000;
const LEAD_CAPTURE_SOURCE_MAX = 80;
...
function normalizePlanPayload(payload: any): Plan {
  const revision = Number.isFinite(Number(payload?.revision))
    ? Math.max(1, Number(payload.revision))
    : 1;
  const nodes = Array.isArray(payload?.graph?.nodes)
    ? payload.graph.nodes
        .filter((node: any) => typeof node?.id === "string" && node.id.trim().length > 0)
        .map((node: any) => ({
          id: node.id.trim(),
          type: node.type === "quantum" ? "quantum" : "classical",
          description: typeof node.description === "string" && node.description.trim().length > 0
            ? node.description
            : "Operational phase node",
          policy_tag: typeof node.policy_tag === "string" && node.policy_tag.trim().length > 0
            ? node.policy_tag.trim()
            : undefined,
          policy_source: typeof node.policy_source === "string"
            ? (node.policy_source === "uacp_internal" || node.policy_source === "model" || node.policy_source === "fallback"
              ? node.policy_source
              : "uacp_internal")
            : undefined,
          mapping_reason: typeof node.mapping_reason === "string"
            ? node.mapping_reason
            : undefined,
          confidence: node.confidence === "low" || node.confidence === "medium" || node.confidence === "high"
            ? node.confidence
            : undefined,
          entropy: typeof node.entropy === "number" && Number.isFinite(node.entropy) ? node.entropy : undefined,
        }))
        .filter((node: any, index: number, array: any[]) => index === array.findIndex((entry) => entry.id === node.id))
    : [];
  ...
  return {
    ...
    name: typeof payload?.name === "string" && payload.name.trim().length > 0 ? payload.name : "AI Generated Plan",
    intent: typeof payload?.intent === "string" && payload.intent.trim().length > 0 ? payload.intent : "No intent available",
    revision,
    status: payload?.status === "verified" || payload?.status === "locked" || payload?.status === "draft"
      ? payload.status
      : "draft",
    graph: {
      nodes: nodes.length > 0 ? nodes : [{
        ...
        type: "classical",
        description: "Operational phase fallback node",
      }],
      edges,
    },
    research: payload?.research,
    createdAt: typeof payload?.createdAt === "string" && payload.createdAt.trim().length > 0 ? payload.createdAt : new Date().toISOString(),
  };
}

function sanitizeRunPlan(plan: unknown): Plan | undefined {
  if (!plan || typeof plan !== "object") {
    return undefined;
  }
  return normalizePlanPayload(plan as any);
}
...
function isAllowedCorsOrigin(origin: string) {
  ...
  if (CORS_ALLOWED_ORIGINS.has(normalized)) {
    return true;
  }

  return CORS_ALLOWED_ORIGINS.has(normalized.replace("https://www.", "https://"))
    || CORS_ALLOWED_ORIGINS.has(normalized.replace("http://www.", "http://"));
}
...
function isTrustedPublicDemoOrigin(request: IncomingMessage | express.Request) {
  ...
  return isAllowedCorsOrigin(origin)
    || /^https:\/\/(www\.)?barbankz\.com\b/i.test(origin)
...
function sanitizeLeadSource(value: string) {
  const normalized = value.trim().slice(0, LEAD_CAPTURE_SOURCE_MAX);
  return normalized.length > 0 ? normalized : "landing_page";
}
...
function normalizeLeadCapture(lead: LeadCaptureEntry) {
  return {
    id: lead.id,
    ...
    ipFingerprint: lead.ipFingerprint ?? null,
  };
}

function hydrateLeadCapturesFromRows(rows: Array<{
  id: string | null;
  email: string | null;
  source: string | null;
  ...
  return rows
    ...
    .map((row) => ({
      ...
      source: sanitizeLeadSource(row.source),
      createdAt: typeof row.created_at === "string"
        ? row.created_at
        : new Date(row.created_at).toISOString(),
      userAgent: typeof row.user_agent === "string" ? row.user_agent : null,
      ipFingerprint: typeof row.ip_fingerprint === "string" ? row.ip_fingerprint : null,
    }))
    .slice(0, LEAD_CAPTURE_MAX_SAFE);
}

async function persistLeadCaptureToPostgres(lead: LeadCaptureEntry) {
  if (!persistencePool) {
    return;
  }
  ...
  try {
    await persistencePool.query(
      `
        insert into lead_captures (id, email, source, created_at, user_agent, ip_fingerprint, last_seen_at)
    ...
    console.error("Postgres lead persistence error:", error);
  }
}

async function loadLeadCapturesFromPostgres(): Promise<LeadCaptureEntry[]> {
  if (!persistencePool) {
    return [];
  }
  ...
  const result = await persistencePool.query(
    `
      select id, email, source, created_at, user_agent, ip_fingerprint
      from lead_captures
      order by created_at desc
      limit $1
    `,
    [LEAD_CAPTURE_MAX_SAFE]
  );

  return hydrateLeadCapturesFromRows(result.rows);
}
...
function applyLoadedState(parsed: Partial<AppState>) {
  const loadedPlans = Array.isArray(parsed.plans)
    ? parsed.plans.map((plan) => normalizePlanPayload(plan))
    : [];

  plans = loadedPlans;
  runs = Array.isArray(parsed.runs) ? parsed.runs.map((run) => {
    const matchedPlan = loadedPlans.find((plan) => plan.id === run.planId);
    return {
      ...run,
      planSnapshot: sanitizeRunPlan(run.planSnapshot) || (matchedPlan ? deepClone(matchedPlan) : undefined),
    };
  }) as Run[] : [];
  events = Array.isArray(parsed.events) ? parsed.events : [];
  archiveRecords = Array.isArray(parsed.archiveRecords) ? parsed.archiveRecords : [];
  publicDemoUsage = parsed.publicDemoUsage || {};
  leadCaptures = Array.isArray(parsed.leadCaptures) ? parsed.leadCaptures
    .filter((lead): lead is LeadCaptureEntry => (
    ...
    .map((lead) => ({
      ...lead,
      userAgent: typeof lead.userAgent === "string" ? lead.userAgent : null,
      ipFingerprint: typeof lead.ipFingerprint === "string" ? lead.ipFingerprint : null,
    }))
    .slice(0, LEAD_CAPTURE_MAX_SAFE)
    : [];
...
async function initializePersistence() {
  ...
  try {
    ...
    const dbLeads = await loadLeadCapturesFromPostgres();

    if (result.rows[0]?.state_json) {
      applyLoadedState(result.rows[0].state_json);
    }
    if (dbLeads.length > 0) {
      leadCaptures = dbLeads;
    }
...
async function startServer() {
  ...
  const PORT = Number(process.env.PORT || "43000");
  ...
  app.post("/api/leads", async (req, res) => {
    const email = normalizeLeadEmail(typeof req.body?.email === "string" ? req.body.email : "");
    const source = sanitizeLeadSource(typeof req.body?.source === "string" ? req.body.source : "landing_page");
    ...
    const leadPayload: LeadCaptureEntry = {
      id: "",
      email,
      source,
      createdAt: now,
    ...
    const existingIndex = leadCaptures.findIndex((lead) => lead.email === email);
    if (existingIndex >= 0) {
      leadPayload.id = leadCaptures[existingIndex].id;
      leadCaptures[existingIndex] = {
        ...leadCaptures[existingIndex],
        ...leadPayload,
        source,
      };
    } else {
      leadPayload.id = crypto.randomUUID();
      leadCaptures = [
        leadPayload,
        ...leadCaptures,
      ];
    ...
    if (leadCaptures.length > LEAD_CAPTURE_MAX_SAFE) {
      leadCaptures = leadCaptures.slice(0, LEAD_CAPTURE_MAX_SAFE);
    }

    const latestLead = normalizeLeadCapture(leadCaptures[existingIndex >= 0 ? existingIndex : 0]);
    if (persistencePool) {
      await persistLeadCaptureToPostgres(latestLead);
    } else {
      persistState();
    }
  ...
  app.get("/api/leads", async (req, res) => {
    ...
    if (persistencePool) {
      leadCaptures = await loadLeadCapturesFromPostgres();
    }
  ...
  app.get("/health", (_req, res) => {
    res.json({
      ok: true,
      status: "operational",
      service: "barbankz-freestylewriter",
      timestamp: new Date().toISOString(),
      persistenceMode,
    });
  });
  ...
  app.get("/api/plans", (req, res) => {
    res.json(plans.map((plan) => normalizePlanPayload(plan)));
  });
  ...
  app.get("/api/plans/:planId", (req, res) => {
    ...
    res.json(normalizePlanPayload(plan));
  });
  ...
  app.post("/api/plans", (req, res) => {
    ...
    res.status(201).json(normalizePlanPayload(newPlan));
  });
  ...
  app.post("/api/plans/compile", async (req, res) => {
    ...
    try {
      ...
      plans.push(newPlan);
      if (res.locals.demoUsageKey) {
        markDemoUsage("planCompiles", res.locals.demoUsageKey);
      }
  ...
  app.get("/api/runs", (req, res) => {
    res.json(runs.map((run) => ({
      ...run,
      planSnapshot: run.planSnapshot ? normalizePlanPayload(run.planSnapshot) : undefined,
    })));
  });
  ...
  app.get("/api/runs/:runId/plan", (req, res) => {
    ...
    res.json(normalizePlanPayload(run.planSnapshot));
  });
  ...
  app.post("/api/runs", (req, res) => {
    ...
    runs.push(newRun);
    if (res.locals.demoUsageKey) {
      markDemoUsage("runStarts", res.locals.demoUsageKey);
    }
