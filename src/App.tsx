...
import { saveTextFile } from "./lib/text-export";
...
export default function App() {
  ...
  const [canDownloadExports, setCanDownloadExports] = useState(true);
  const [leadEmail, setLeadEmail] = useState("");
  const [leadMessage, setLeadMessage] = useState<string | null>(null);
  const [leadSubmitting, setLeadSubmitting] = useState(false);
  const socketRef = useRef<WebSocket | null>(null);
  const executionViewportRef = useRef<HTMLDivElement | null>(null);
  const normalizeGraphEdges = (edges: unknown): Plan["graph"]["edges"] => {
  ...
  const normalizeNode = (value: any): Plan["graph"]["nodes"][number] => {
    const raw = value ?? {};
    const entropyValue = typeof raw.entropy === "number" && Number.isFinite(raw.entropy)
      ? raw.entropy
      : 0.18;
    return {
      id: typeof raw.id === "string" && raw.id.trim().length > 0 ? raw.id : `node-${Math.random().toString(36).slice(2, 8)}`,
      type: raw.type === "quantum" ? "quantum" : "classical",
      description: typeof raw.description === "string" && raw.description.trim().length > 0
        ? raw.description
        : "Operational phase node",
      policy_tag: typeof raw.policy_tag === "string" && raw.policy_tag.trim().length > 0 ? raw.policy_tag.trim() : undefined,
      policy_source: typeof raw.policy_source === "string" ? raw.policy_source as "uacp_internal" : undefined,
      mapping_reason: typeof raw.mapping_reason === "string" ? raw.mapping_reason : undefined,
      confidence: raw.confidence === "low" || raw.confidence === "medium" || raw.confidence === "high" ? raw.confidence : undefined,
      entropy: entropyValue,
    };
  };
  const normalizeGraphNodes = (nodes: unknown): Plan["graph"]["nodes"] => {
    ...
    const normalized = nodes
      ...
      .filter((node, index, array) => index === array.findIndex((entry) => entry.id === node.id));

    return normalized;
  };
  const normalizePlan = (payload: any): Plan => {
    const graph = payload?.graph ?? {};
    return {
      ...payload,
      id: typeof payload?.id === "string" && payload.id.trim().length > 0 ? payload.id : `plan-${crypto.randomUUID()}`,
      name: typeof payload?.name === "string" && payload.name.trim().length > 0 ? payload.name : "Generated Plan",
      intent: typeof payload?.intent === "string" ? payload.intent : "No intent available",
      revision: Number.isFinite(payload?.revision) ? Number(payload.revision) : 1,
      status: payload?.status === "verified" || payload?.status === "locked" || payload?.status === "draft" ? payload.status : "draft",
      createdAt: typeof payload?.createdAt === "string" ? payload.createdAt : new Date().toISOString(),
      graph: {
        nodes: normalizeGraphNodes(graph?.nodes),
  ...
  const normalizePlans = (payload: unknown): Plan[] => {
    ...
    return payload.map((entry) => normalizePlan(entry));
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const authPayload = await authRes.json().catch(() => ({ authenticated: false, authRequired: false, authMode: "disabled" })) as AuthStatusPayload;
        setCanDownloadExports(Boolean(authPayload.authenticated) || authPayload.authMode === "disabled");
        if (!authRes.ok) {
  ...
  const resolveRunPlan = (run: Run) => {
    if (run.planSnapshot) {
      return normalizePlan(run.planSnapshot);
    }

    const matchedPlan = plans.find((plan) => plan.id === run.planId);
    return matchedPlan ? normalizePlan(matchedPlan) : null;
  };
  ...
  const downloadPlanSchema = (plan: Plan, scope: "active" | "run" = "active") => {
    try {
      ...
      void saveTextFile(JSON.stringify(payload, null, 2), `${safeName}-${plan.id}-schema.json`);
  ...
  const downloadArtifact = (run: Run) => {
    ...
    try {
      ...
      void saveTextFile(JSON.stringify(payload, null, 2), `${run.artifact.artifactId}.json`);
  ...
  const downloadReplay = async (run: Run) => {
    try {
      const replay = await fetchReplay(run.id);
      void saveTextFile(JSON.stringify(replay, null, 2), `${replay.replayId}.json`);
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      ...
      setLeadEmail("");
      setLeadMessage("You are on the list.");
    } catch (error) {
      setLeadMessage(error instanceof Error ? error.message : "Email submission failed");
    } finally {
  ...
  const pressure = typeof signals?.uacp_pressure === "number" ? signals.uacp_pressure : 0;
  ...
  const activeResearch = activePlan?.research ?? selectedPlan?.research ?? null;
  const researchCoverage = activeResearch
    ? Math.max(
        0,
        Math.min(
          1,
          0.45 +
          ((activeResearch.topics.length / 4) * 0.3) +
          (activeResearch.synthesis ? 0.15 : 0) +
          (activeResearch.sourceSignals.length > 0 ? 0.1 : 0)
        )
      )
    : (ssrnData.length > 0 ? 0.5 : 0);
  const activePlanNodes = normalizeGraphNodes(activePlan?.graph?.nodes);
  const selectedPlanNodes = normalizeGraphNodes(selectedPlan?.graph?.nodes);
  const selectedPlanEdges = normalizeGraphEdges(selectedPlan?.graph?.edges);
  const policyCoverage = activePlanNodes.length
    ? activePlanNodes.filter((node) => node.policy_tag?.trim()).length / activePlanNodes.length
    : 0;
  ...
  return (
    <div className="h-screen flex flex-col bg-[#050505] text-[#e0e0e0] font-sans selection:bg-blue-500/30 overflow-hidden relative">
      ...
      <main className={`flex-1 grid gap-1 p-1 bg-white/5 overflow-hidden ${blackBoxMode ? "grid-cols-1" : "grid-cols-12"}`}>
        ...
        <section className={`${blackBoxMode ? "col-span-1 max-w-[1120px] w-full mx-auto my-3 rounded-[24px] shadow-[0_30px_80px_rgba(0,0,0,0.45)]" : "col-span-6"} flex flex-col bg-[#080808] border border-white/5 overflow-hidden technical-grid relative`}>
                ...
                <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto text-center space-y-12">
                  ...
                  {blackBoxMode && (
                    <div className="w-full max-w-3xl">
                      <div className="flex flex-wrap items-center justify-center gap-2 rounded-full border border-white/8 bg-black/35 px-3 py-2 backdrop-blur-sm">
                        <HubMetricPill label="Pressure" value={`${Math.round(pressure * 100)}%`} accent="amber" />
                        <HubMetricPill label="Certainty" value={certaintyIndex.toFixed(4)} accent="blue" />
                        <HubMetricPill label="Prime" value={`${Math.round(primeReadiness * 100)}%`} accent="emerald" />
                        <HubMetricPill label="Research" value={`${Math.round(researchCoverage * 100)}%`} accent="violet" />
                        <HubMetricPill label="Policy" value={`${Math.round(policyCoverage * 100)}%`} accent="slate" />
                      </div>
                    </div>
                  )}
                  <p
                  ...
                  <div className="mt-8 w-full max-w-xl mx-auto">
                    <p className="text-[9px] uppercase tracking-[0.3em] text-white/30 mb-3">
                      Start collecting early-access emails
                    </p>
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter" && !leadSubmitting) {
                              e.preventDefault();
                              void handleJoinWaitlist();
                            }
                          }}
                          placeholder="you@domain.com"
                          className="w-full pl-9 pr-4 py-3 bg-black/80 border border-white/10 text-sm text-white/85 focus:outline-none focus:border-blue-500/50"
                        />
                      </div>
                      <button
                        onClick={() => void handleJoinWaitlist()}
                        disabled={leadSubmitting || !leadEmail.trim()}
                        className="px-4 py-3 border border-blue-500/30 bg-blue-500/10 text-[10px] uppercase tracking-[0.2em] font-mono text-blue-100 hover:bg-blue-500/20 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        {leadSubmitting ? "Saving" : "Save"}
                      </button>
                    </div>
                    {leadMessage && (
                      <p className="mt-2 text-[9px] uppercase tracking-[0.3em] text-blue-300/80">
                        {leadMessage}
                      </p>
                    )}
                  </div>
              ...
              >
                <div className="flex justify-between items-center mb-12 border-b border-white/5 pb-6">
                   ...
                   <div className="flex gap-4">
                     ...
                     <button
                      ...
                      disabled={!activePlan || !canDownloadExports}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-white/60 leading-relaxed italic">
                          "{activePlan.intent}" - Sequence initialized with {activePlanNodes.length || 0} nodes.
                ...
                {activePlan ? (
                  ...
                  <div className="flex min-w-max items-start gap-12 relative animate-in fade-in duration-700">
                    ...
                    <div className="flex flex-col items-center justify-center gap-6 opacity-40 py-16">
                      <Layers size={48} className="animate-pulse" />
                      <span className="font-serif italic text-lg">Compiled graph has no renderable nodes.</span>
                    </div>
                  )
                ) : (
                        ...
                        <div className="flex justify-between items-start mb-8">
                          <div className="space-y-2">
                              ...
                              <div className="flex gap-3 pt-2">
                                ...
                                <button
                                  ...
                                  disabled={!runPlan || !canDownloadExports}
                                ...
                                <button
                                  ...
                                  disabled={!run.artifact || !canDownloadExports}
                         ...
                         <div className="space-y-4">
                           ...
                           <div className="flex justify-between mt-2 overflow-hidden">
                              {runPlan ? (() => {
                                const runPlanNodes = normalizeGraphNodes(runPlan.graph.nodes);
                                return runPlanNodes.length > 0 ? runPlanNodes.map((node, nIdx) => (
                                  <div key={`${node.id}-${nIdx}`} className="flex flex-col items-center gap-1 opacity-20 hover:opacity-100 transition-opacity cursor-default">
                                    <div className={`w-1 h-1 rounded-full ${nIdx / (runPlanNodes.length || 1) * 100 <= run.progress ? 'bg-blue-400' : 'bg-white/40'}`} />
                                    <span className="text-[7px] font-mono uppercase tracking-tighter">{node.id}</span>
                                  </div>
                                )) : (
                                  <span className="text-[9px] font-mono uppercase tracking-[0.2em] text-white/35">No rendered plan nodes for this run.</span>
                                );
                              })() : null}
                            </div>
                        </div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
              ...
              <div className="p-6 border-r border-white/10 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {selectedPlanNodes.map((node, index) => (
                    <div key={`${node.id}-${index}`} className="border border-white/10 p-4 bg-white/[0.02]">
                      <div className="flex items-center justify-between gap-4">
                        <span className="text-sm font-mono text-white/90 uppercase">{node.id}</span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="border border-white/10 p-4 bg-white/[0.02] space-y-2">
                    {selectedPlanEdges.map((edge, index) => (
...
function HubMetricPill({
  ...
  const accentClasses: Record<typeof accent, string> = {
    ...
    blue: "border-blue-400/15 bg-blue-400/[0.04] text-blue-200",
    ...
    slate: "border-white/10 bg-white/[0.03] text-white/85",
  };

  return (
    <div className={`rounded-full border px-3 py-1.5 ${accentClasses[accent]}`}>
      <div className="flex items-center gap-2">
        <span className="text-[8px] font-mono uppercase tracking-[0.18em] text-white/35">{label}</span>
        <span className="text-[13px] font-serif italic leading-none">{value}</span>
      </div>
    </div>
  );
}
